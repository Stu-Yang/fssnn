...
def train(args, model, private_train_loader, optimizer, epoch):   # 训练函数
    ...
    for batch_idx, (data, target) in enumerate(private_train_loader):    # 小批量训�?
        ...
        def forward(optimizer, model, data, target):    # 前向传播，其定义了每次执行的计算步骤，最后返回loss函数
            ...
            if args.model in {"network2", "alexnet", "vgg16"}:           # 定义loss函数（不同网络结构的loss函数不一样）
                loss_enc = output.cross_entropy(target)                  # loss函数为交叉熵函数
            else:
                batch_size = output.shape[0]
                loss_enc = ((output - target) ** 2).sum() / batch_size   # loss函数为均方函数
...
@profile  # 使用@Profile注解实现不同的开发环境使用不同的数据�?
