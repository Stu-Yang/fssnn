...
import torch.nn.functional as F    #包含torch.nn库中所有函数，比如loss和activation function
import torchvision.models as models


class Network1(nn.Module):      # nn.Module是PyTorch体系下所有神经网络模块的基类，在定义自己的网络时需要重新实现构造函数__init__构造函数和forward这两个方法
    def __init__(self, dataset, out_features):     #初始化模块内部状态
        super(Network1, self).__init__()
        self.fc1 = nn.Linear(784, 128)              # class torch.nn.Linear(in_features, out_features, bias=True)，这里是对输入数据做线性变换：y=Ax+b
...
def load_state_dict(model, model_name, dataset):  # 将预训练的参数权重加载到新的模型之中
