...
from procedure_copy import train, test          # procedure_copy instead of procedure
...
if __name__ == "__main__":
    ...
    # 模型训练
    model = FCNN()
    model.encrypt(**kwargs)     # Overloads fix_precision for torch.nn.Module.
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        (training_times_epoch, training_comm_epoch) = train(args, model, private_train_loader, optimizer, epoch)

        training_times.append(training_times_epoch)
        training_comm.append(training_comm_epoch)
    ...
    print("Online training time: {:.5f}s, and online training comm. is {:.5f}MB"
         .format(torch.tensor(training_times).mean().item(),
                 torch.tensor(training_comm ).mean().item()))
    print("================================================")
